...
We will use RStudio, and the packages [`roxygen2` from the functions episode][ep-func], as well as [`usethis`][usethis], [`testthat`][testthat]. These make creating packages in R relatively simple. They can be installed from CRAN like this:


~~~
install.packages(c("roxygen2", "testthat", "usethis"))
...
> Find out how to add this information!
> >
> > ## Hint
> > 
...
> > `date: 2018-07-12`.
> > 
> > The ORCiD has to be inserted in `person()` as `comment = c(ORCID = "0000-…-…-…")`.
> {: .solution}
{: .challenge}
