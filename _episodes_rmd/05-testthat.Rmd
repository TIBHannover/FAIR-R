...
> > ## Hint
> > 
> > The examples should be instructive in order to help users to figure out how
> > to apply your functions. The tests should be fast in order to not break the
> > flow when programming, but there should also be enough tests to cover
> > edge-cases, errors, etc. Also note that [examples are run during `R CMD check`](https://klutometis.github.io/roxygen/articles/rd.html#functions)
> > and thus automatically tested as well. Therefore, duplicating them is not
> > necessary, but acceptable if it is both an instructive example and a fast
> > test case.
> > 
> {: .solution}
{: .challenge}
