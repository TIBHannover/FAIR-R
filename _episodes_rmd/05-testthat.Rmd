---
title: "Unit-testing and refactoring"
teaching: 30
exercises: 0
questions:
...
objectives:
- "Describe the required R package structure for testing."
- "Create the required R package structure for testing."
- "Use testthat functions to create and run tests."
- "Changing code is not always necessary, but often useful."
keypoints:
- "Examples can be formalised into unit-tests."
...
Unit test are another way to "span a safety net", and in R, the [`testthat` package][testthat]
is commonly used. Because adding tests requires an expansion of the folder and file
structure of an R package, we are going to use a second helper package: [`usethis`][usethis].

Because we installed and loaded both packages alongside `roxygen2` earlier, you
...
A new file should be created called and pre-filled with a little example. While
...
is an explanation of what this tests actually tests. In case of our `center()`
function we expect that "centering works".

Next comes a specific `expect_`ation, usually that calling a given function with
a defined set of arguments (inputs) is `_equal` to a known return value (outputs).
...
tradition, `testthat` does not bother you confirmations that things are working
...
for example as "Span safety net for TDD & refactoring".
